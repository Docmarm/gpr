...
def detecter_anomalies_geolocalisation(
    ...
    if not trajets_suspects.empty:
        for idx, row in trajets_suspects.iterrows():
            date_str = row['Date_Heure_Debut'].split()[0] if ' ' in row['Date_Heure_Debut'] else row['Date_Heure_Debut']
...
def afficher_page_analyse_vehicules(df_transactions: pd.DataFrame, df_vehicules: pd.DataFrame, date_debut: datetime.date, date_fin: datetime.date, kpi_cat: pd.DataFrame):
    ...
    selected_vehicle = st.selectbox("Sélectionner un véhicule", ["Tous"] + all_vehicles)
    
    if selected_vehicle != "Tous":
        # Obtenir les informations du véhicule
        vehicle_info = df_vehicules[df_vehicules['Nouveau Immat'] == selected_vehicle].iloc[0]
        ...
        if not vehicle_transactions.empty:
            total_volume = vehicle_transactions['Quantity'].sum()
            total_amount = vehicle_transactions['Amount'].sum()
            nb_transactions = len(vehicle_transactions)
            
            st.subheader("Statistiques de Consommation")
            col4, col5, col6 = st.columns(3)
            col4.metric("Volume Total", f"{total_volume:.1f} L")
            col5.metric("Montant Total", f"{total_amount:,.0f} CFA")
            col6.metric("Nombre de Transactions", f"{nb_transactions}")
            ...
            afficher_dataframe_avec_export(
                ...
                key=f"vehicle_transactions_{selected_vehicle}"
            )
            
            # Graphique d'évolution de la consommation
            st.subheader("Évolution de la Consommation")
            vehicle_transactions['date'] = vehicle_transactions['DateTime'].dt.date
            daily_consumption = vehicle_transactions.groupby('date')['Quantity'].sum().reset_index()
        ...
        if not kpi_cat.empty:
            ...
            afficher_dataframe_avec_export(kpi_cat, "KPIs par Catégorie", key="kpi_cat_overview")
        ...
        vehicle_stats = merged_df.groupby(['Nouveau Immat', 'Catégorie']).agg(
            Volume_Total=('Quantity', 'sum'),
            Montant_Total=('Amount', 'sum'),
            Nb_Transactions=('Quantity', 'count')
        ).reset_index()
        
        st.subheader("Statistiques par Véhicule")
        afficher_dataframe_avec_export(
            vehicle_stats.sort_values('Volume_Total', ascending=False), 
            "Statistiques par Véhicule", 
            key="vehicle_stats_all"
        )
